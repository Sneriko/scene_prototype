...
def test_transcribe_audio_loads_audio_before_calling_asr(monkeypatch, tmp_path: Path) -> None:
    captured_input = {}

    def fake_asr(audio_input, **kwargs):
        captured_input["audio_input"] = audio_input
        captured_input.update(kwargs)
        return {"text": " hej "}

    backend = LocalKBWhisperBackend.__new__(LocalKBWhisperBackend)
    backend._asr_pipeline = fake_asr
    monkeypatch.setattr(
        LocalKBWhisperBackend,
        "_load_audio_for_asr",
        lambda self, audio_path: {"array": [0.0, 0.1], "sampling_rate": 16_000},
    )

    transcript = backend.transcribe_audio(tmp_path / "recording.wav")

    assert transcript == "hej"
    assert captured_input["audio_input"] == {"array": [0.0, 0.1], "sampling_rate": 16_000}
    assert captured_input["generate_kwargs"] == {"language": "sw"}

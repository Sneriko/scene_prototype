...
class LocalKBWhisperBackend(OpenAIBackend):
    ...
    def transcribe_audio(self, audio_path: Path) -> str:
        result = self._asr_pipeline(self._load_audio_for_asr(audio_path), generate_kwargs={"language": "sw"})
    ...
    def diarize_transcript(self, raw_transcript: str, audio_path: Path | None = None) -> DiarizedTranscript:  # type: ignore[override]
        ...
        asr_result = self._asr_pipeline(self._load_audio_for_asr(audio_path), return_timestamps=True, generate_kwargs={"language": "sw"})
    ...
    def _load_audio_for_asr(self, audio_path: Path) -> dict[str, Any]:
        """Load audio into memory so Transformers does not need server-side FFmpeg decoding."""
        samples, sample_rate = self._read_audio_file(audio_path)
        mono_samples = samples.mean(axis=1)
        return {"array": mono_samples, "sampling_rate": sample_rate}
    ...
    def _read_audio_file(self, audio_path: Path):
        try:
            import soundfile as sf
        except ImportError as exc:
            raise RuntimeError(
                "Local audio loading requires 'soundfile'. "
                "Install local ASR dependencies with: pip install -e '.[local_asr]' (or pip install -e '.[edge]' for the edge server)"
            ) from exc
        ...
        try:
            ...
            raise RuntimeError(
                f"Could not decode {audio_path.name!r} without server-side FFmpeg. "
                "Use the browser frontend WAV recorder or upload PCM WAV/FLAC/OGG audio supported by soundfile."
            ) from exc
